...
const SHELL_CommandTypeDef shellDefaultCommandList[] =
{
    ...
    SHELL_CMD_ITEM(test, shellTest, test command),
};
...
void shellInit(SHELL_TypeDef *shell)
{
    ...
    shell->keyFuncBase = NULL;
    shell->keyFuncNumber = 0;
    #if SHELL_USING_VAR == 1
...
static void shellEnter(SHELL_TypeDef *shell)
{
    ...
    *(shell->buffer + shell->length++) = 0;
    ...
    shellDisplay(shell, "\r\n");
    base = (SHELL_CommandTypeDef *)shell->commandBase;
...
void shellTest(void)
{
    ...
    shellDisplay(shell, "hello world--");
    shellDisplayItem(shell, 2);
    shellDisplay(shell, "\033[2J\033[1H");
    return;
}
SHELL_EXPORT_CMD(test, shellTest, clear command line);